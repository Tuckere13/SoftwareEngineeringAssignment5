...
def create_resource(resource: schemas.RecipeCreate, db: Session = Depends(get_db)):
    return resources.create(db=db, resource=resource)


@app.get("/resources/", response_model=list[schemas.Recipe], tags=["Resources"])
def read_resource(db: Session = Depends(get_db)):
    return resources.read_all(db)


@app.get("/resources/{resource_id}", response_model=schemas.Recipe, tags=["Resources"])
...
@app.post("/order_details/", response_model=schemas.Recipe, tags=["Details"])
def create_order_detail(order_detail: schemas.RecipeCreate, db: Session = Depends(get_db)):
    return order_details.create(db=db, order_detail=order_detail)


@app.get("/order_details/", response_model=list[schemas.Recipe], tags=["Details"])
def read_order_detail(db: Session = Depends(get_db)):
    return order_details.read_all(db)


@app.get("/order_details/{order_detail_id}", response_model=schemas.Recipe, tags=["Details"])
def read_one_order_detail(order_detail_id: int, db: Session = Depends(get_db)):
    ...
    if order_detail is None:
        raise HTTPException(status_code=404, order_detail="User not found")
    return order_detail


@app.put("/order_details/{order_detail_id}", response_model=schemas.Recipe, tags=["Details"])
def update_one_order_detail(order_detail_id: int, order_detail: schemas.RecipeUpdate, db: Session = Depends(get_db)):
...
@app.delete("/order_details/{order_detail_id}", tags=["Details"])
def delete_one_order_detail(order_detail_id: int, db: Session = Depends(get_db)):
    order_detail = order_details.read_one(db, order_detail_id=order_detail_id)
    if order_detail is None:
        raise HTTPException(status_code=404, detail="User not found")
    ##return details.delete(db=db, details_id=details_id)